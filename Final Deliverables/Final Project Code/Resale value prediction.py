...
def dashboard():
    if 'user' in session:
        ...
        return render_template('history.html', name=user['name'],history=user['history'])
    else:
...
def predict():
    if 'user' in session:
        return render_template('predict.html',modelData=modelData, fuelData=fuelData, vehicleData=vehicleData, brandData=brandData)
    else:
        return redirect('/auth')
